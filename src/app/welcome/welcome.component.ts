...
export class WelcomeComponent implements OnInit {
  ...
  isLoadingRoadWorkActivities: boolean = false;

  roadWorkNeedColumns: string[] = [
    'status',
    'areaman',
    'title',
    'person',
  ...
  roadWorkActivityColumns: string[] = [
    'status',
    'area_man',
    'title',
    'involved',
  ...
  columnDefsNeed: ColDef[] = [
    {
      ...
      cellRenderer: ({ data }: any) => {
        ...
        const map: {
          ...
          verified1: {
            label: '12/verifiziert-1',
            color: '#64b5f6'
          },
          verified2: {
            label: '12/verifiziert-2',
            color: '#64b5f6'
          },
          inconsult1: {
            label: '12/Bedarfsklärung - 1.Iteration',
            color: '#4fc3f7'
          },
          inconsult2: {
            label: '12/Bedarfsklärung - 2.Iteration',
            color: '#4fc3f7'
          },
          reporting: {
            label: '12/Stellungnahme',
            color: '#29b6f6'
          },
          coordinated: {
            label: '12/koordiniert',
            color: '#0288d1'
          },
          prestudy: {
            label: '21/Vorstudie',
    ...
    },
    {
      ...
      cellRenderer: ({ data }: any) => {
        ...
        const href =
          `https://stadtplan.winterthur.ch?topic=Grundkarte&scale=1000&x=${x}&y=${y}&back=Hintergrundkarte_LK_AV_Situationsplan`;
    ...
    },
    {
      ...
      cellRenderer: ({ data }: any) => {
        ...
        const href =
          `http://intramap.winport.net/projekte/tiefbau_info/start_redirect_wikis.php?&x=${x}&y=${y}`;
  ...
  columnDefsActivity: ColDef[] = [
    ...
    },
    {
      ...
      valueGetter: ({ data }) =>
        data?.properties?.roadWorkActivityNo || '',
    ...
    },
    {
      ...
      cellRenderer: ({ data }: any) => {
        ...
        return `
          <a
            href="/civil-engineering/roadworks-portal/activities/${uuid}"
            title="${tooltip}"
            style="
              display: inline-block;
              white-space: nowrap;
  ...
  @ViewChild('agGridNeed')
  agGridNeed!: AgGridAngular;

  @ViewChild('agGridActivity')
  agGridActivity!: AgGridAngular;
  ...
  ngOnInit(): void {
    ...
    if (this.userService.isUserLoggedIn()) {
      ...
      this.roadWorkNeedService
        ...
        .subscribe({
          next: (roadWorkNeeds) => {
            ...
            const myRoadWorkNeeds:
              Map<string, RoadWorkNeedFeature> =
              new Map();

            for (const roadWorkNeed of roadWorkNeeds) {

              this.managementAreaService
                .getIntersectingManagementArea(
                  roadWorkNeed.geometry
                )
                .subscribe({
                  next: (managementArea) => {

                    if (managementArea) {
                      roadWorkNeed.properties.managementArea =
                        managementArea;

                      setTimeout(() => {
                        if (
                          this.agGridNeed?.api?.refreshCells
                        ) {
                          this.agGridNeed.api.refreshCells({
                            force: true
                          });
                  ...
                  error: (error) => {
                    console.error(
                      'Error while loading management area for requirement',
                      error
                    );
                  }
                });
              ...
              if (
                roadWorkNeed.properties.status !==
                'coordinated'
              ) {
                if (
                  roadWorkNeed.properties.orderer.uuid ===
                  loggedInUser.uuid
                ) {
                  myRoadWorkNeeds.set(
                    roadWorkNeed.properties.uuid,
                    roadWorkNeed
                  );
                }
              }
            ...
            this.myRoadWorkNeedFeatures =
              Array.from(
                myRoadWorkNeeds.values()
              );
          },

          error: (error) => {
            console.error(
              'Error while loading requirements',
              error
            );
          }
        });
      ...
      this.isLoadingRoadWorkActivities = true;

      this.roadWorkActivityService
        .getRoadWorkActivities(
          '',
          'requirement,review,inconsult1,inconsult2,reporting,verified1,verified2,coordinated,prestudy,suspended'
        )
        .subscribe({

          next: async (roadWorkActivities) => {

            try {

              const myRoadWorkActivities:
                Map<string, RoadWorkActivityFeature> =
                new Map();

              for (
                const roadWorkActivity
                of roadWorkActivities
              ) {

                // Determine the area manager first.
                // The request is asynchronous, therefore it must
                // finish before the user relationship is evaluated.
                try {
                  ...
                  const managementArea =
                    await firstValueFrom(
                      this.managementAreaService
                        .getIntersectingManagementArea(
                          roadWorkActivity.geometry
                        )
                    );

                  if (managementArea) {
                    roadWorkActivity.properties.areaManager =
                      managementArea.manager;
                  }

                } catch (error) {

                  console.error(
                    'Fehler beim Ermitteln des Gebietsmanagements für Bauvorhaben',
                    roadWorkActivity.properties?.uuid,
                    error
                  );
                }

                const isAreaManager =
                  roadWorkActivity.properties
                    .areaManager?.uuid ===
                  loggedInUser.uuid;
                ...
                if (
                  ...
                  myRoadWorkActivities.set(
                    roadWorkActivity.properties.uuid,
                    roadWorkActivity
                  );
                }

                // Also display a road work activity when a linked,
                // not yet coordinated requirement belongs to the
                // currently logged-in user.
                const roadWorkNeedsUuids =
                  roadWorkActivity.properties
                    .roadWorkNeedsUuids;

                if (roadWorkNeedsUuids?.length) {
                  ...
                  try {
                    ...
                    console.error(
                      'Fehler beim Laden der verknüpften Bedarfe für Bauvorhaben',
                      roadWorkActivity.properties?.uuid,
                      error
                    );
                  }
              ...
              this.myRoadWorkActivityFeatures =
                Array.from(
                  myRoadWorkActivities.values()
                );

              // Prepare organisations of involved users
              // for display in the grid.
              this.involvedOrgs.clear();

              for (
                const roadWorkActivityFeature
                of this.myRoadWorkActivityFeatures
              ) {

                for (
                  const involvedUser
                  of roadWorkActivityFeature.properties
                    .involvedUsers ?? []
                ) {

                  const organisationalUnit =
                    involvedUser?.organisationalUnit;

                  if (organisationalUnit?.uuid) {
                    this.involvedOrgs.set(
                      organisationalUnit.uuid,
                      organisationalUnit
                    );
                  }
                }
              ...
              if (
                this.agGridActivity?.api
                  ?.refreshCells
              ) {
                this.agGridActivity.api.refreshCells({
                  force: true
                });
              }
               ...
               * also if an asynchronous operation fails.
               */
              this.isLoadingRoadWorkActivities = false;
            }
          },
          ...
          error: (error) => {
            ...
            console.error(
              'Fehler beim Laden der Bauvorhaben',
              error
            );
          }
        });
          ...
          next: (users) => {
            ...
            if (
              ...
              if (
                ...
                this.user.chosenRole =
                  localUser.chosenRole;
              }
            }
          },
  ...
  update(
    ...
    this.user.passPhrase =
      this.user.passPhrase.trim();

    if (
      changePassphrase &&
      this.user.passPhrase.length === 0
    ) {
  ...
  getColorDueDate(
    ...
    if (roadworkActivity) {
      ...
      if (dueDate) {
        ...
        const threeDaysBeforeDue:
          Date =
          new Date(dueDate);

        threeDaysBeforeDue.setDate(
          dueDate.getDate() - 3
        );

        const oneDayAfterDue =
          new Date(dueDate);

        oneDayAfterDue.setDate(
          dueDate.getDate() + 1
        );

        if (
          today >= oneDayAfterDue
        ) {
          return 'background-color: rgb(255, 109, 109);';
        } else if (
          today >= threeDaysBeforeDue
        ) {
  ...
  openWinWebGIS() {
    ...
    window.open(
      'http://intramap.winport.net/projekte/tiefbau_info/start_redirect.php',
      '_blank'
    );
  }
  ...
  calcDueDate(
    ...
    if (
      roadworkActivity.properties.status ===
      'inconsult1' ||
      roadworkActivity.properties.status ===
      'verified1' ||
      roadworkActivity.properties.status ===
  ...
  getInvolvedOrgsNames(
    ...
    if (roadWorkActivity) {
      ...
      for (
        const involvedUser
        of roadWorkActivity.properties
          .involvedUsers
      ) {
